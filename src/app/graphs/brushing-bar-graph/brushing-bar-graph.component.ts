...
export class BrushingBarGraphComponent implements OnInit {
  ...
  loadGraph() {
    ...
    const svg = d3.select('.brushing-bar-graph').append('svg')
    ...
    const context = svg.append('g')
      ...
      const subBars = context.selectAll('.subBar')
                                        .data(data);
    ...
    function brushed() {
      ...
      function updateScale(data) {
        ...
        const filteredTickValues = data.filter(function(d, i) {return i % tickValueMultiplier === 0; }).map(function(d) { return d.date; });
